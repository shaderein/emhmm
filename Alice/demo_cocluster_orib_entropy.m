...
OPT = struct;
taskType = 2; % 1: vehicle
SETTING.save_var = "yes"; % "yes" if you want to save variables; "no" if you don't want to save variables
...
if taskType == 1
    outindmat = 'veh_individual_hmms_300.mat';
    outgrpmat = 'veh_vbcogroup_hmms_300_run.mat';
else
    outindmat = 'hum_individual_hmms.mat';
    outgrpmat = 'hum_vbcogroup_hmms.mat';
end
...
switch(OPT.datasource)
    ...
    case 10
        ...
        if taskType == 1
            OPT.fixationfile = 'ORIB-data/veh_id_fix_12_07_2022_11_00.xlsx';
            OPT.imgdir       = 'ORIB-data/orib_veh_id_task_resized/';
            OPT.imgsize      = [1024; 576];
            OPT.imginfo      = 'ORIB-data/veh_id_stimuli_info.xlsx';
            OPT.outdir       = 'ORIB-data-vb-results-lab-veh-300-rerun/';
        else
            OPT.fixationfile = 'ORIB-data/hum_id_fix_12_17_2022_17_58.xlsx';
            OPT.imgdir       = 'ORIB-data/orib_hum_id_task_resized/';
            OPT.imgsize      = [1024; 576];
            OPT.imginfo      = 'ORIB-data/hum_id_stimuli_info.xlsx';
            OPT.outdir       = 'ORIB-data-vb-results-lab-hum/';
        end

        % individual HMM settings (set vbopt like this)
        OPT.S               = 1:5;  % number of ROIs to try
        OPT.vbopt.numtrials = 300;

        % co-clustering settings
        OPT.vb_cocluster    = 0;     % vb-coclustering
        OPT.HEM_K           = 2;   % try these numbers of clusters
        OPT.HEM_S           = [];    % use median
        OPT.SWAP_CLUSTERS   = []; % swap the cluster IDs for consistency w/ paper
        OPT.vbhemopt.trials = 300;    % options for vb co-clustering
        ...
        do_cluster = 0;    % run co-clustering (set to 0 to reuse results)
...
if SETTING.save_var == "yes"
    if taskType == 1
        save(fullfile(OPT.outdir,'veh_useful_variables_analysis_spilt_half.mat'),...
            "AB","KL1","KL2","LL","LL1","LL1_grp1","LL1_grp2","LL2","LL2_grp1",...
            "LL2_grp2","StimuliNamesC","Entropies","H");
    else
        save(fullfile(OPT.outdir,'hum_useful_variables_analysis_spilt_half.mat'),...
            "AB","KL1","KL2","LL","LL1","LL1_grp1","LL1_grp2","LL2","LL2_grp1",...
            "LL2_grp2","StimuliNamesC","Entropies","H");
    end

else
    disp(['Note: some useful variables will not be saved in a mat file. ' ...
        'Please change the setting variable if you want to save them in a mat file.'])
end
